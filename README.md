...
> `data/vegetables/*.md` 由 `getStaticProps` 在构建时直接读取，没有中间产物。
> dev 下每次请求都会重新读，改完 `.md` 刷新页面就能看到。
...
lib/vegetables.ts    类型与常量，客户端也能 import
lib/vegetables.server.ts  读 .md 的解析与校验，只能在 getStaticProps 里用
...
改完直接提交 `.md` 就行，没有需要一起生成的产物。

frontmatter 只支持上面这几种写法（标量 + 缩进列表），不是完整 YAML。
字段拼错、类型不对、图片文件不存在、`order` 撞车，构建都会指名报错，不会静默通过
（解析与校验都在 `lib/vegetables.server.ts`）。

### 改价格

对应 `.md` 里改 `price`。单位是**日元／公斤**，整数。
...
字段不全或图片路径写错时，`npm run build` 会报错，照着提示补。
